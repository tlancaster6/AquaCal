...
must_haves:
  truths:
    ...
    - "All plots specified in the brief are present (no dropped plots included)"
...
2. **RIG_SIZE toggle cell**:
   ```python
   RIG_SIZE = "small"  # "small" (4 cameras, 10 frames, fast) or "large" (13 cameras, 30 frames, compelling results)
   ```
...
4. **Preset configuration cell**: Define presets based on RIG_SIZE:
   - "small": 4 cameras (use `generate_synthetic_rig("small")` or `create_scenario("small")`), 10 frames, 3 test depths for exp2/3
   - "large": 13 cameras (use `generate_real_rig_array()`), 30 frames, full depth sweeps
...
6. **Experiment 2: Depth Generalization** (4-6 cells)
   ...
   - Code cell: generate 13-camera rig (large) or smaller rig (small), calibrate on narrow depth band (0.95-1.05m), evaluate at multiple test depths. For "small" preset use 3 test depths (e.g., 0.85, 1.0, 1.5); for "large" use full sweep [0.80, 0.90, 1.00, 1.10, 1.20, 1.40, 1.70, 2.00].
...
```
The small preset (4 cameras, 10 frames) should complete in under 5 minutes. If it fails, fix the notebook.
