...
must_haves:
  truths:
    ...
    - "InterfaceParams is re-exported from the bridge so call sites never import from aquakit directly"
    - "Conversion helpers _to_torch() and _to_numpy() are private to the bridge"
  artifacts:
    - path: "src/aquacal/core/_aquakit_bridge.py"
      provides: "Numpy↔torch conversion wrappers for all 5 AquaKit geometry functions"
      exports:
...
**Re-export AquaKit types at the top** so call sites import from bridge, never from aquakit directly:
```python
from aquakit.types import InterfaceParams  # re-export
```
...
from aquakit.types import InterfaceParams  # noqa: F401 — re-exported for call sites
from aquacal.config.schema import Vec2, Vec3
from aquacal.core.camera import Camera
```

**`__all__`** should list all public bridge functions and `InterfaceParams`.
...
Each bridge function must have a Google-style docstring with Args, Returns, and a note that it converts numpy↔torch internally.
  ...
  <verify>
Run: `python -c "from aquacal.core._aquakit_bridge import _bridge_snells_law_3d, _bridge_refractive_project, InterfaceParams; print('bridge imports OK')" 2>&1`
...
Also run: `python -m ruff check src/aquacal/core/_aquakit_bridge.py` to confirm no lint errors.
  ...
  <done>
`src/aquacal/core/_aquakit_bridge.py` exists with all 5 bridge functions and InterfaceParams re-export. Each function has a Google-style docstring, correct type hints, and matches the AquaCal numpy API for its inputs and outputs. Ruff passes with no errors.
  </done>
</task>
...
- `InterfaceParams` re-exported from bridge
- `_to_torch` and `_to_numpy` are private (prefixed with `_`)
- `__all__` is defined listing bridge functions and `InterfaceParams`
- File has module docstring
- `python -m ruff check src/aquacal/core/_aquakit_bridge.py` passes
</verification>

<success_criteria>
Bridge module created and importable. All 5 wrapper functions present with correct numpy-in/numpy-out signatures. InterfaceParams re-exported. Ruff clean.
</success_criteria>
