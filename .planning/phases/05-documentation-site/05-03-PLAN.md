...
<task type="auto">
  <name>Task 2: Improve public API docstrings with examples and cross-links</name>
  <files>
    src/aquacal/core/refractive_geometry.py
    src/aquacal/core/camera.py
    src/aquacal/core/board.py
  ...
  <action>
  Review and improve docstrings for all PUBLIC functions and classes. Focus on:
  ...
  **Priority 3 — Cross-links to theory pages:**
  - In key functions, add a "See Also" or "Note" referencing the theory page
  - Example: In `snells_law_3d` docstring, add:
    ```
    Note:
  ...
  **Important:** Do NOT modify function signatures, logic, or behavior. Only docstrings.
  ...
  <verify>
  `python -m sphinx docs docs/_build -b html` builds without errors. Spot-check key API pages to verify: (1) usage examples appear in rendered docs, (2) cross-links to theory pages render as clickable links, (3) all public functions show complete Args/Returns documentation.
  Run `python -m pytest tests/ -m "not slow" -x` to ensure docstring changes didn't break anything.
  </verify>
  <done>
  Key functions have 3-5 line usage examples in docstrings. All public functions/classes have complete Args/Returns documentation. Cross-links from API docs to theory pages work. Tests still pass.
  </done>
</task>
