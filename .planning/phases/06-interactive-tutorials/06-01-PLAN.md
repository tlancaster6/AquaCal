...
must_haves:
  ...
  key_links:
    - from: "src/aquacal/io/detection.py::detect_all_frames"
      to: "src/aquacal/io/detection.py::_create_frame_source"
      via: "detect_all_frames calls _create_frame_source when given dict[str, str] paths"
      pattern: "_create_frame_source"
    - from: "src/aquacal/io/detection.py::_create_frame_source"
      to: "src/aquacal/io/images.py::ImageSet"
      via: "auto-detection: Path.is_dir() -> ImageSet, Path.is_file() -> VideoSet"
      pattern: "is_dir.*ImageSet|is_file.*VideoSet"
...
<feature>
  ...
  <implementation>
    **Important wiring note:** No changes to config loading or schema.py are needed.
    The current pipeline flow is: config.yaml -> dict[str, str] paths -> detect_all_frames().
    detect_all_frames() already receives paths as dict[str, str] and internally creates
    the frame source. The auto-detection (directory = images, file = video) happens
    entirely within detection.py's _create_frame_source, which is called by
    detect_all_frames when it receives a dict of paths.
