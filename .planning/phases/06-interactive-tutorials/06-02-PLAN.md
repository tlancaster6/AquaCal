...
<task type="auto">
  ...
  <action>
    0. **Verify docs pages exist before linking.** Before writing README links, check
       which docs pages actually exist. As of planning time, the docs structure is:
       - Methodology/theory: `docs/guide/` has refractive_geometry.md, coordinates.md, optimizer.md (link here for "methodology")
       - API reference: `docs/api/` has calibration.rst, config.rst, core.rst, io.rst, etc. (link here for "API reference")
       - Tutorials: `docs/tutorials/index.md` (link here for "tutorials")
       - CLI reference: **No dedicated page exists.** Link to the overview page (`docs/overview.md`) or the Getting Started section that covers CLI usage. Do NOT link to a non-existent CLI reference page.
       - Config reference: **No dedicated page exists.** Link to `docs/api/config.rst` (the config module API docs) as the closest equivalent.
       If any of these pages are missing at execution time, adjust links to point to the nearest existing content rather than creating broken links.
  ...
  <verify>
    ...
    # Verify all docs link targets exist (no broken links to non-existent pages)
    for link_target in docs/guide/index.md docs/api/index.rst docs/tutorials/index.md docs/overview.md; do
      test -f "$link_target" && echo "OK: $link_target" || echo "MISSING: $link_target"
    done
    ```
    README is concise with hero visual and docs links. All linked docs pages exist. Tutorial index updated.
  </verify>
